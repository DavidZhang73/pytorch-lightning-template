...
from pytorch_lightning.cli import SaveConfigCallback, LightningCLI, LightningArgumentParser
...
class CustomLightningCLI(LightningCLI):
    """
    Custom LightningCLI that
    1. Saves the config file to the log directory
    2. Load the default config file
    3. Print the path of the loaded checkpoint after testing
    4. Add an argument to apply a test right after fit based on the best checkpoint
    5. Add an argument to ignore warnings
    """
    ...
    def __init__(
        ...
        super().__init__(
            ...
            parser_kwargs=new_parser_kwargs,
            **kwargs
        )
    ...
    def add_arguments_to_parser(self, parser: LightningArgumentParser) -> None:
        ...
        parser.add_argument('--git_commit_before_fit', default=False, type=bool,
                            help='Git commit before training')
        parser.add_argument('--test_after_fit', default=False, type=bool,
    ...
    def after_fit(self) -> None:
        if self.config.fit.get('test_after_train'):
    ...
    def _prepare_subcommand_kwargs(self, subcommand: str) -> Dict[str, Any]:
        ...
        fn_kwargs = {
            k: v for k, v in self.config_init[self.config_init['subcommand']].items() if
            k in self._subcommand_method_arguments[subcommand]
        }
